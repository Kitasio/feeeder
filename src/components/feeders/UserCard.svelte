    ...
    <div>
        <div>
            <div class="flex space-x-4 items-center">
                <img class="w-16 h-16 rounded-full border-2 border-green" src="https://avatars.dicebear.com/api/jdenticon/${account.toLowerCase()}.svg" alt="">
                <div>
                    <p class="text-xs font-medium opacity-80">Fred</p>
            ...
            <div class="mt-2">
                ...
                {:then balance}
                    <div class="text-5xl text-green font-sans font-bold">{balance}<span class="text-base font-sans ml-1">ETH</span></div>
                {/await}
            </div>
        ...
        <div class="flex justify-between mt-3">
            <a class="rounded-3xl font-medium px-5 py-2 border hover:border-green hover:bg-green hover:text-gray-dark transition duration-200" href={`/withdraw/${$page.params.address}`}>Withdraw</a>
        </div>
    </div>
    ...
    <div class="py-2 hidden xl:flex flex-col justify-between">
        ...
        <div>
            ...
            {#await totalIncome()}
                Waiting...
            {:then value} 
                <span class="font-sans text-lg font-bold text-green">{value}<span class="text-sm ml-1">ETH</span></span>
            {/await}
        </div>
        <div>
            ...
            {#await getUser($page.params.address, account)}
                Waiting...
            {:then user} 
                <span class="font-sans text-lg font-bold text-red">{parseFloat((user.withdrawn / 10**18).toFixed(4))}<span class="text-sm ml-1">ETH</span></span>
            {/await}
        </div>
    </div>
</div>