...
export const getAllMembers = async (feederAddress: string) : Promise<string[]> => {
...
export const getName = async (feederAddress: string) : Promise<string> => {
    const contract = new Web3.eth.Contract(feederABI, feederAddress)
    return await contract.methods.name().call()
}

// Returns the balance of a Feeder
export const getBalance = async (feederAddress: string) : Promise<string> => {
    const contract = new Web3.eth.Contract(feederABI, feederAddress)
...
export const getTotalFunded = async (feederAddress: string) : Promise<string> => {
    const contract = new Web3.eth.Contract(feederABI, feederAddress)
    return await contract.methods.getTotalFunded().call()
}

// Returns the user object of a Feeder
export const getUser = async (feederAddress: string, userAddress: string) : Promise<string> => {
    const contract = new Web3.eth.Contract(feederABI, feederAddress)
    return await contract.methods.user(userAddress).call()
}

// Returns all the Feeders related to a users wallet
export const getAddresses = async (userAddress: string) : Promise<string[]> => {
...
export const avaliableBalance = async (feederAddress: string, userAddress: string) : Promise<number> => {
    const total = await getTotalFunded(feederAddress)
    const user = await getUser(feederAddress, userAddress)
    const balance = ((parseInt(total) * parseInt(user['allocation'])) / 100) - parseInt(user['withdrawn'])
    return parseFloat((balance/10**18).toFixed(4))
}

// Calculates the avaliable balance of a Feeder for specified user in WEI
export const avaliableBalanceInWEI = async (feederAddress: string, userAddress: string) : Promise<number> => {
    const total = await getTotalFunded(feederAddress)
    const user = await getUser(feederAddress, userAddress)
    const balance = ((parseInt(total) * parseInt(user['allocation'])) / 100) - parseInt(user['withdrawn'])
    return balance
...
export const withdrawInETH = async (eth: number, feederAddress: string, userAddress: string) : Promise<string> => {
    const contract = new Web3.eth.Contract(feederABI, feederAddress)
    const wei = eth * 10**18
    const avaliableBalance = await avaliableBalanceInWEI(feederAddress, userAddress)
